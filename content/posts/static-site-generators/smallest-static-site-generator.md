...
description: Tired of bloat, I made a new static site generator that is 5x faster and 50x smaller than my last effort. And this one runs on 32-bit NetBSD.
date: 2025-04-22
keywords: [lua]
---
Today, I unveil my most gratuitous static site generator yet: [luasmith](https://github.com/jaredkrinke/luasmith).

* **It's 450 KB** (static binary, including templates for this site, gzip compressed)
...
* **It's trivial to compile**, requiring only a C compiler, GNU Make, `sed`, and `sh`
* Despite the small size, it still includes **[syntax highlighting for ~150 languages](luasmith-syntax-highlighting.md) and [internal link-checking](extending-luasmith.md)**
...
Note: templates use a Lua-native templating language called [etlua](https://github.com/leafo/etlua) and file matching uses Lua [patterns](https://www.lua.org/manual/5.2/manual.html#6.4.1).
...
[The (previous) best static site generator ever made](https://jaredkrinke.github.io/md2blog/) is built on TypeScript/JavaScript and [Deno](https://deno.com/). As much as I like Deno, I'm not confident it will be maintained decades down the road. Given that I'll never know how to get it running on an i586 computer, I have doubts I'd be able to get it running on potential future architectures (RISC-V?) either.
...
* [Scintillua](https://github.com/orbitalquark/scintillua) for syntax highlighting
* [LPeg](https://www.inf.puc-rio.br/~roberto/lpeg/) to support Scintillua
...
* Syntax highlighting is *not* simple -- good news: [Scintillua already solved this problem for me](luasmith-syntax-highlighting.md)!
...
Distributing native code necessarily requires per-platform packages. Or does it? Can I package and release this minimal static site generator as a multi-OS polyglot binary using [Cosmopolitan libc](https://github.com/jart/cosmopolitan)? **Update**: this now exists, but sadly, it is 64-bit only.
...
* [Example site: a blog of things I've learned](https://jaredkrinke.github.io/til/) ([source](https://github.com/jaredkrinke/til))

Note: the example repository contains [a simple self-contained luasmith pipeline for generating its README file](https://github.com/jaredkrinke/til/blob/main/readme.lua).
